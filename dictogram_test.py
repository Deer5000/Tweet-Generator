...
class DictoTest(unittest.TestCase):
    ...
    def test_entries(self):
        dictogram = Dicto(self.fish_words)
    ...
    def test_contains(self):
        histogram = Dicto(self.fish_words)
    ...
    def test_frequency(self):
        ...
        assert histogram.freq('one') == 1
        assert histogram.freq('two') == 1
        assert histogram.freq('red') == 1
        assert histogram.freq('blue') == 1
        assert histogram.freq('fish') == 4
        # Verify frequency count of unseen words
        assert histogram.freq('food') == 0
    ...
    def test_add_count(self):
        ...
        assert histogram.freq('one') == 1
        assert histogram.freq('two') == 3
        assert histogram.freq('red') == 1
        assert histogram.freq('blue') == 4
        assert histogram.freq('fish') == 8
        assert histogram.freq('food') == 5
